...
describe('TSMT Basic Data Stats', () => {
  ...
  it('does not accept a null data provider', function() {
    let myStats: TSMT$DataStats = new TSMT$DataStats();
    myStats.data = null;
  ...
  it('reports correct sample covariance', function() {
    let myStats: TSMT$DataStats = new TSMT$DataStats();
    let cov: number             = myStats.covariance( [2.1, 2.5, 4.0, 3.6], [8, 12, 14, 10] );
  ...
  it('reports correct Pearson corr. coef.', function() {
    let myStats: TSMT$DataStats = new TSMT$DataStats();
    let corr: number             = myStats.correlation( [43, 21, 25, 42, 57, 59, 247], [99, 65, 79, 75, 87, 81, 486] );
