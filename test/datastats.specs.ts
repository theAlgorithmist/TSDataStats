...
import {TSMT$DataStats} from '../src/DataStats';

import * as Chai from 'chai';
const expect = Chai.expect;

// Test Suites
describe('TSMT Basic Data Stats', () => {
  ...
  it('reports correct sample covariance', function() {
    const stats: TSMT$DataStats = new TSMT$DataStats();
    let cov: number             = stats.covariance( [2.1, 2.5, 4.0, 3.6], [8, 12, 14, 10] );
  ...
  it('reports correct Pearson corr. coef.', function() {
    const stats: TSMT$DataStats = new TSMT$DataStats();
    let corr: number            = stats.correlation( [43, 21, 25, 42, 57, 59, 247], [99, 65, 79, 75, 87, 81, 486] );
  ...
  it('returns correct covariance matrix from 5 x 3 input', function() {
    const stats: TSMT$DataStats       = new TSMT$DataStats();
    const input: Array<Array<number>> = new Array<Array<number>>();
    ...
    const cov: Array<Array<number>> = stats.covarianceMatrix(input);
